...
static void ArrayIterator_setval(ArrayIteratorClass *this, ...)
{
    ...
    if (this->_idx >= this->_array->_size)
        raise("Out of range");
    va_start(ap, this);
    this->_array->_tab[this->_idx] = new(this->_array->_type, val);
    val = va_arg(ap, Object *);
    va_end(ap);
}
...
static void Array_ctor(ArrayClass *this, va_list *args)
{
    ...
    this->_tab = calloc(this->_size + 1, sizeof(Object *));
    if (this->_tab == NULL)
        raise("Out of memory");
    for (size_t i = 0; i < this->_size; i++) {
        va_copy(temp, *args);
        this->_tab[i] = va_new(this->_type, &temp);
    }
}
...
static void Array_setitem(ArrayClass *this, ...)
{
    ...
    pos = va_arg(ap, size_t);
    val = va_arg(ap, Object *);
    this->_tab[pos] = new(this->_type, val);
    va_end(ap);
}
