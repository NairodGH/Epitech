...
import { useCryptosControllerGetCryptoHistoryQuery } from "@/store/api/backendApi/endpoints/v1";
import {
  List,
  ListItem,
  Radio,
...
import { getCoordArray } from "@/utils/curveOperations";
import dynamic from "next/dynamic";
import { Period, getOptions } from "@/utils/chartOptions";
...
export default function CryptoGraph(props: CryptoGraphProps) {
  ...
  const { data: cryptos, isLoading } =
    useCryptosControllerGetCryptoHistoryQuery(
      { cmid: id, period: dateType },
      { refetchOnMountOrArgChange: true }
    );

  useEffect(() => {
    if (cryptos) {
      setCurveData(getCoordArray(cryptos.priceHistory));
    }
  }, [cryptos]);
    ...
    <Container>
      <div className="flex flex-col gap-10">
        ...
        <div>
          <ReactECharts
            option={getOptions(
              cryptos?.lowestPriceDay ?? 0,
              cryptos?.highestPriceDay ?? 0,
              curveData,
              dateType
            )}
          />
