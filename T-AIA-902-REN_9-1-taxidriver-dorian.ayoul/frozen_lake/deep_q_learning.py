...
def train():
    ...
    for episode in range(episodes):
        ...
        while not done and steps < max_steps:
            ...
            reward = 10 if reward == 1 else -5 if done else -0.1
    ...
    torch.save(model.state_dict(), f"frozenlake_dqn_{timestamp}.pt")
    print(f"Model saved as frozenlake_dqn_{timestamp}.pt")
    ...
    # plt.title('Deep Q-learning Performance in FrozenLake-v1')
    # plt.grid()
    # plt.show()

def run(model):
    env = gym.make("FrozenLabke-v1", desc=["SFFF", "FHFH", "FFFH", "HFFG"], map_name="4x4", render_mode="human", is_slippery=True)
    state_size = env.observation_space.n
    max_steps = 100
    state = env.reset()[0]
    state = torch.eye(state_size)[state]
