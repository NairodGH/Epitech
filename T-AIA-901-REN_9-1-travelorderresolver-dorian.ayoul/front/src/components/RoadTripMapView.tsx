...
import {EventDetail, PlaceApiResResult, PlaceDetail} from "../types";
...
const RoadTripMapView: React.FC<Props> = ({width, height, zoomDef, margin}) => {
    ...
    const searchHotel = async () => {
        ...
        if(hotel.data.results) {
            ...
            return createCustomMarker(results, 'hotel')
        }
    }
    ...
    const searhBars = async () => {
        ...
        if(bars.data.results) {
            ...
            return createCustomMarker(results, 'bars')
        }
    }
    ...
    const searhRestaurants = async () => {
        ...
        if(restaurant.data.results) { 
            ...
            return createCustomMarker(results, 'restaurant')
        }
    }
    ...
    const searchEvents= async () => {
        ...
        if(events.data.events) {
            ...
            return createCustomMarker(results, 'events')
        }
    }
    ...
    const handlePoi = async () => {
        ...
        if(isRestaurantsSearching) {
            const res = await searhRestaurants()
            if(res !== undefined) {
    ...
    const mapOnChange = () => {
        if(mapRef != null) {
            ...
            const newCenter = {lat: mapRef.current?.getCenter()?.lat(), lng: mapRef.current?.getCenter()?.lng() }
    ...
    const addWaypoints = (item: PlaceDetail | null | undefined, event: EventDetail | null | undefined) => {
        if(directionsOptions) {
            const currentDirectionsOptions = JSON.parse(JSON.stringify(directionsOptions))
    ...
    const handleSaveTrip = async () => {
        ...
        waypointsDetails.map((wp: any) => {
            const wpData = fullWp.find((e) => e.place_id === wp.place_id)
            placesToSave.push({
                name: wp.name, 
...
export default RoadTripMapView
